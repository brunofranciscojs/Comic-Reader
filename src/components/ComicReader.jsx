...
export default function ComicReader({ file, setOverlay, overlay, progress }) {
  ...
  return (
     (overlay &&
      <div className="fixed backdrop-blur-md top-0 left-0 w-full h-dvh flex justify-center items-center z-50 [background:var(--bg)_#000d] bg-blend-multiply !bg-center !bg-[length:200px]" style={{"--bg":`url(/../assets/${bg}.jpg)`}}>
        ...
        <div className="flex justify-between absolute left-1/2 -translate-x-1/2 top-0 z-30 w-full px-12 py-5 backdrop-blur-md bg-black/40">
          <h2 className="text-xl font-bold">{file.fileName.split('(')[0] || "Sem arquivo"}</h2>

          <button className="bg-black text-white rounded-full w-5 h-5" onClick={closeSlider}>
            X
          </button>
        </div>
          {slider ? (<Splide options={{ perPage: 1, arrows: true, pagination:true }} className="[&>#splide01-track]:h-[90dvh] flex h-dvh">
            {file.images.map((image, index) => (
              <SplideSlide key={index}>
                ...
                <span className="text-white px-2 py-1 bg-black/30 absolute top-4 left-4 ">Página {index + 1}</span>
              </SplideSlide>
            ))}
          </Splide>) : (
